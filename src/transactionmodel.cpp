...
namespace Etherwall {
    ...
    void TransactionModel::sendTransactionDone(const QString& hash) {
        ...
        ShiftLog::logMsg("Transaction sent hash: " + hash);
    }
        ...
        foreach ( const QString bns, list ) {
            ...
            if ( val.contains("{") ) { // new format, get data and reload only recent transactions
                ...
                if ( parseError.error != QJsonParseError::NoError ) {
                    ShiftLog::logMsg("Error parsing stored transaction: " + parseError.errorString(), LS_Error);
                } else {
    ...
    void TransactionModel::loadHistory() {
        ...
        ShiftLog::logMsg("HTTP Post request: " + data, LS_Debug);
    ...
    void TransactionModel::loadHistoryDone(QNetworkReply *reply) {
        ...
        ShiftLog::logMsg("HTTP Post reply: " + data, LS_Debug);

        QJsonParseError parseError;
        const QJsonDocument resDoc = QJsonDocument::fromJson(data, &parseError);

        if ( parseError.error != QJsonParseError::NoError ) {
            return ShiftLog::logMsg("Response parse error: " + parseError.errorString(), LS_Error);
        }
        ...
        foreach ( const QJsonValue jv, result ) {
            ...
            if ( hash == "bogus" ) {
                return ShiftLog::logMsg("Response hash missing", LS_Error);
            }
